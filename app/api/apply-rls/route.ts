...
export async function POST() {
  ...
  if (!supabaseUrl || !supabaseServiceRoleKey) {
    return NextResponse.json({ error: 'Supabase URL or service role key is not defined' }, { status: 500 });
  }
  ...
  return NextResponse.json({ message: 'RLS policies applied successfully!' });
}
